...
int genmove(int Board[BOUNDARYSIZE][BOUNDARYSIZE], 
    ...
    return_move = pick_move(Board, 
                            turn, game_length, time_limit, 
                            GameRecord,
                            num_legal_moves, MoveList);
...
int gen_legal_move(int Board[BOUNDARYSIZE][BOUNDARYSIZE], 
    ...
    for (int y = 1 ; y <= BOARDSIZE; ++y) {
        ...
        if (Board[x][y] == 0) {
            ...
            }else{// Case 2: no empty intersection in the neighborhood
...
void gtp_genmove(int Board[BOUNDARYSIZE][BOUNDARYSIZE], 
                 char Color[], int time_limit, int game_length, 
                 int GameRecord[MAXGAMELENGTH][BOUNDARYSIZE][BOUNDARYSIZE]){
